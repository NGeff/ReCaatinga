...
const MemoriaHibridaGame = ({ game, onComplete, onExit }) => {
  ...
  if (cards.length === 0) return <div className="game-card"><div className="game-error">Nenhum par disponível.</div></div>;
  ...
  return (
    <div className="game-card">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: `repeat(${gridSize}, 1fr)`, gap: '1rem', marginBottom: '2rem', maxWidth: '700px', marginLeft: 'auto', marginRight: 'auto', position: 'relative', zIndex: 1 }}>
        {cards.map((card) => {
          ...
          return (
            <div key={card.id} style={{ aspectRatio: '1', cursor: 'pointer', perspective: '1000px' }} onClick={() => handleCardClick(card.id)}>
              <div style={{ position: 'relative', width: '100%', height: '100%', transition: 'transform 0.6s', transformStyle: 'preserve-3d', transform: isFlipped ? 'rotateY(180deg)' : 'rotateY(0)' }}>
                ...
                <div style={{ position: 'absolute', width: '100%', height: '100%', backfaceVisibility: 'hidden', background: 'white', border: matchedCards.includes(card.id) ? '2px solid var(--game-green)' : '2px solid var(--game-border)', transform: 'rotateY(180deg)', borderRadius: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0.75rem', boxShadow: '0 4px 12px rgba(0,0,0,0.1)', overflow: 'hidden' }}>
                  {card.type === 'image' ? (
                    <img src={card.content} alt="Card" style={{ width: '90%', height: '90%', objectFit: 'cover', borderRadius: '0.5rem' }} />
                  ) : (
                    <div style={{ fontSize: '0.85rem', fontWeight: '600', color: 'var(--game-text)', textAlign: 'center', lineHeight: '1.4', padding: '0.5rem' }}>{card.content}</div>
                  )}
                </div>
              </div>
            </div>
...
export default MemoriaHibridaGame;
